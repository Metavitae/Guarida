...
import {
  ...
  Plus, Stethoscope, ScrollText, ChevronRight,
} from "lucide-react";
...
// A small local sample standing in for the real legal_references table —
// same shape as the seed data already drafted for Nayarit.
const SAMPLE_LEGAL_REFS = [
  {
    id: "1",
    statute_code: "CPN Art. 422",
    title: "Maltrato o crueldad animal (Nayarit)",
    keywords: ["golpes", "golpe", "abandono", "agua", "alimento", "herida", "heridas", "crueldad", "maltrato"],
  },
  {
    id: "2",
    statute_code: "CPN Art. 423",
    title: "Agravantes — grabación o publicación del maltrato",
    keywords: ["video", "grabó", "grabaron", "publicó", "fotos", "foto"],
  },
  {
    id: "3",
    statute_code: "LGVS Art. 29-30",
    title: "Trato digno a fauna silvestre",
    keywords: ["silvestre", "salvaje", "iguana", "tortuga", "ave", "venado", "coyote"],
  },
];

function suggestMatches(description) {
  const words = description.toLowerCase();
  return SAMPLE_LEGAL_REFS
    .map((ref) => ({
      ...ref,
      score: ref.keywords.filter((k) => words.includes(k)).length,
    }))
    .filter((r) => r.score > 0)
  ...
  return (
    <div className="mb-6">
      <label
        ...
        className="block text-xs tracking-wide uppercase mb-2 opacity-70"
      >
        {label}
      </label>
      {children}
...
export default function CaseIntake() {
  ...
  const matches = useMemo(() => suggestMatches(description), [description]);
  ...
  return (
    <div style={{ backgroundColor: COLORS.paper, minHeight: "100vh", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ backgroundColor: COLORS.night }} className="px-6 md:px-12 py-5 flex items-center gap-3">
        ...
        <span style={{ fontFamily: "'Fraunces', serif", color: COLORS.paper }} className="text-lg">
          Guarida
        </span>
      ...
      <div className="max-w-2xl mx-auto px-6 py-12">
        <h1 style={{ fontFamily: "'Fraunces', serif", color: COLORS.ink }} className="text-3xl mb-2">
          Report a case
        </h1>
        ...
        <Field label="Description">
          <textarea
            style={inputStyle}
            className="w-full rounded-xl px-4 py-3 text-sm outline-none min-h-32 resize-none"
        ...
        <Field label="Evidence">
          <button
            style={{ border: `1.5px dashed ${COLORS.line}`, color: `${COLORS.ink}88` }}
        ...
        <button
          onClick={() => setRequiresVet(!requiresVet)}
          style={{
            backgroundColor: requiresVet ? `${COLORS.coral}18` : "#FFFFFF",
            border: `1.5px solid ${requiresVet ? COLORS.coral : COLORS.line}`,
          }}
        ...
        >
          ...
          <span style={{ color: COLORS.ink }} className="text-sm flex-1">
            This case needs veterinary attention
          </span>
          <div
            style={{
              backgroundColor: requiresVet ? COLORS.coral : COLORS.line,
            }}
            className="h-5 w-9 rounded-full relative transition-colors"
          >
            <motion.div
              animate={{ x: requiresVet ? 16 : 2 }}
              className="absolute top-0.5 h-4 w-4 rounded-full bg-white shadow"
            />
          </div>
        </button>

        {/* LEGAL SUGGESTIONS — live, never auto-confirmed */}
        <AnimatePresence>
          {matches.length > 0 && (
            <motion.div
              ...
              style={{ backgroundColor: COLORS.nightDeep }}
              className="rounded-2xl p-6 mb-8 overflow-hidden"
            >
              <div className="flex items-center gap-2 mb-1">
                <ScrollText size={16} color={COLORS.marigold} />
                <span style={{ color: COLORS.paper, fontFamily: "'IBM Plex Mono', monospace" }} className="text-xs uppercase tracking-wide">
                  Suggested legal matches
                </span>
              </div>
              ...
              <div className="space-y-2">
                {matches.map((m) => (
                  <div
                    key={m.id}
                    style={{
                      backgroundColor: confirmed[m.id] === "dismissed" ? "transparent" : `${COLORS.paper}0d`,
                      opacity: confirmed[m.id] === "dismissed" ? 0.4 : 1,
                    }}
                    className="rounded-xl px-4 py-3 flex items-center gap-3"
                  >
                    <div className="flex-1">
                      <div style={{ color: COLORS.paper, fontFamily: "'IBM Plex Mono', monospace" }} className="text-xs mb-0.5">
                        {m.statute_code}
                      </div>
                      <div style={{ color: `${COLORS.paper}b3` }} className="text-sm">
                        {m.title}
                      </div>
                    </div>
                    <button
                      ...
                      style={{
                        backgroundColor: confirmed[m.id] === "confirmed" ? COLORS.marigold : "transparent",
                        border: `1.5px solid ${COLORS.marigold}`,
                        color: confirmed[m.id] === "confirmed" ? COLORS.nightDeep : COLORS.marigold,
                      }}
                    ...
                    <button
                      onClick={() => toggleDismiss(m.id)}
                      style={{
                        backgroundColor: "transparent",
                        border: `1.5px solid ${COLORS.paper}33`,
                        color: `${COLORS.paper}88`,
                      }}
                      className="h-8 w-8 rounded-full flex items-center justify-center shrink-0"
                    >
                      <X size={14} />
        ...
        {requiresVet && (
          <div
            style={{ backgroundColor: `${COLORS.coral}12`, border: `1px solid ${COLORS.coral}44` }}
            className="rounded-xl px-4 py-3 mb-8 flex items-center gap-2"
          >
            <AlertTriangle size={16} color={COLORS.coral} />
            <span style={{ color: COLORS.ink }} className="text-xs">
              Active vets on this case will be notified the moment this is submitted.
            </span>
          </div>
        )}
        ...
        <button
          ...
          className="w-full rounded-full py-4 font-medium text-sm"
        >
